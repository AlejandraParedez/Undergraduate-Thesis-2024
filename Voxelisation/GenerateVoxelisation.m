...
Entranceframe = [Rz(deg2rad(-90)) [8 -50 17]'; 0 0 0 1]; % for easy testing [x r x]

plotcoord3(Entranceframe,10,'r','g','b')

%Labelling the Goal Volume

% % MultiGoal: [position, size]
...
%Plot Trocar Region
hold on
plotprism(Entranceframe,t_ori(1),t_ori(2),t_ori(3),t_dim(1),t_dim(2),t_dim(3),'m')
...
% RED
W = 40 +20;%100; % x_upper-x_lower +20 for easy testing
x_lower = -round(W/2);
x_upper = 0;%round(W/2);

% GREEN
H = 16; % y_upper-y_lower
y_lower = -2; % -8; changed for easy testing
y_upper = round(H/2);

% BLUE
L = 10;%40; % z_upper-z_lower
z_lower = -2;% -8;% round(L/2); changed for easy testing
z_upper =2; % round(L/2) ;%8; changed for easy testing
...
% assume diameter of snake robot
w = 3; 

%Define voxel side cubic Structuring element of voxels
se_x = ceil(w/3); % dx); %e.g. 2 voxel grid = 4/2
se_y = ceil(w/3); % dy);
se_z = ceil(w/3); % dz);
...
c.Limits = [100*round(max_value,3) 0];  %%% SWAPPED ORDER & -1 
c.Ticks = flip(100.*round(linspace(0,max_value,bins-1),3)); %% remove flip
