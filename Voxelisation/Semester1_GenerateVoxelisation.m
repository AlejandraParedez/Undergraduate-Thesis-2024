...
axis('image');
%view([-45 45]);
view([-30 28]); %Best viewing angle of all four targets
...
Entranceframe = [Rz(deg2rad(-90)) [0 -30 10]'; 0 0 0 1]; %  [Ry(deg2rad(-90)) [0 -30 10]'; 0 0 0 1];
...
% g_ori_dim =  [7-10+2, -4.0+30-12, -5.0, 3.0, 3.0, 2.0]; 
g_ori_dim =  [-22, 4, 0, 3.0, 2.0, 3.0]; 
...
%Trocar space
t_ori =  [10-10-10, -25+25-3, -3]' ; %[10, -25, -3]'; % [10-10, -25+25, -3]'; [0 20 2]';
...
L = 10; %25; % z_upper-z_lower
W = 26;%40; % x_upper-x_lower
H = 24;%35;%70; % y_upper-y_lower

%Define scope of Voxel space
% % % % %Real space bounds at rounded limits
% % % % x_lower = 0;%-round(H/2); %-round(W/2); %  ZZZZZZZZ
% % % % x_upper = round(H/2); %round(W/2);
% % % % y_lower = 0;%-round(W/2); %-round(H/2)-15;
% % % % y_upper = round(W/2); %round(H/2)-15;
% % % % z_lower = -round(L/2); %0;
% % % % z_upper = round(L/2); %round(L);


% x_lower = 2-10                ;                %0;%-round(H/2); %-round(W/2); %  ZZZZZZZZ
% x_upper = round(H/2)-10  +10+2         ;             %round(W/2);
% y_lower = -round(W/2)+30     -20       ;   %-round(W/2); %-round(H/2)-15;
% y_upper = 0+30          ;               %round(H/2)-15;
% z_lower = -round(L/2)         ;               %0;
% z_upper = 3        +5;                      %-3;%round(L/2)-3; %round(L);

y_lower = 2-10                ;                %0;%-round(H/2); %-round(W/2); %  ZZZZZZZZ
y_upper = round(H/2)-10  +10+2         ;             %round(W/2);
x_lower = -round(W/2)+30     -20  -20     ;   %-round(W/2); %-round(H/2)-15;
x_upper = 0+30         -30 ;               %round(H/2)-15;
z_lower = -round(L/2)         ;               %0;
z_upper = 3        +5;                      %-3;%round(L/2)-3; %round(L);
...
se_x = ceil(w/8); %e.g. 2 voxel grid = 4/2
se_y = ceil(w/8);
se_z = ceil(w/8);
